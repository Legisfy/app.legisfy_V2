-- Create migration to add gabinete assignments to plans and payment tracking
...
-- Create view for plan assignments with gabinete info
CREATE OR REPLACE VIEW public.plan_assignments AS
SELECT 
  p.id as plan_id,
  ...
  (g.subscription_end_date - now()) as days_until_renewal,
  CASE 
    WHEN g.subscription_end_date - now() <= INTERVAL '7 days' THEN true
    ELSE false
  END as renewal_soon
FROM public.plans p
...
-- Create RLS policy for plan assignments view
CREATE POLICY "Users can view plan assignments for their gabinetes" ON public.plan_assignments
  FOR SELECT 
  USING (
    gabinete_id IS NULL OR 
    user_belongs_to_cabinet(gabinete_id) OR 
    is_platform_admin()
  );