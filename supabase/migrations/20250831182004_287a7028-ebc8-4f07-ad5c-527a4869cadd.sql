-- Primeiro, vamos corrigir a função user_belongs_to_cabinet
...
AS $$
  SELECT EXISTS (
    ...
    WHERE g.id = target_cabinet_id
    AND g.politico_id = auth.uid()
  );
$$;

-- Também vamos atualizar a função get_active_cabinet para ser mais robusta
