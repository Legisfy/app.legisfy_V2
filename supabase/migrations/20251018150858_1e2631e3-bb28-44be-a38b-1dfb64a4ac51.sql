-- Create enum for campaign frequency if not exists
DO $$ BEGIN
...
-- Create table for campaigns if not exists
...
DROP POLICY IF EXISTS "campaigns_cabinet_access" ON public.campaigns;
...
CREATE INDEX IF NOT EXISTS idx_campaigns_gabinete ON public.campaigns(gabinete_id);
CREATE INDEX IF NOT EXISTS idx_campaigns_public ON public.campaigns(public_id);