...
-- Create the enum type for tag types
DO $$ BEGIN
...
-- Update the indicacoes table to include tag reference
...
-- Update existing indicacoes to remove email reference if needed
-- (The orgao_responsavel field can be repurposed for organization name if needed)